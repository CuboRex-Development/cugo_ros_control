...
def generate_launch_description():
    default_ekf_conf       = 'config/gnss_localization/amcl_and_gnss_localization.yaml'
    default_odom_pub_conf  = 'config/gnss_localization/gnss_odom_publisher.yaml'
    
    localization_conf = LaunchConfiguration('ekf_conf_file'      ,default=default_ekf_conf)
    odom_pub_conf     = LaunchConfiguration('gnss_odom_conf_file',default=default_odom_pub_conf)
    use_sim_time      = LaunchConfiguration('use_sim_time'       ,default=False)
    wheel_odom_topic  = LaunchConfiguration('wheel_odom_topic'   ,default='cugo_ros2_control/wheel/odometry')
    global_odom_topic = LaunchConfiguration('global_odom_topic'  ,default='odom')
    ...
    odom_pub_config_fullpath = LaunchConfiguration('odom_pub_config_fullpath')
    
    return LaunchDescription([
        # Parameters
        DeclareLaunchArgument('ekf_conf_file'      ,default_value = default_ekf_conf     ,description = 'ekf parameter file'),
        DeclareLaunchArgument('gnss_odom_conf_file',default_value = default_odom_pub_conf,description = 'gnss odometry publisher parameter file'),
